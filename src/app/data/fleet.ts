export const fleet = [
  {
    name: "MG 3 (Auto)",
    type: "Hatchback",
    specs: { doors: 5, passengers: 5, luggage: "1 Large, 1 Small", engine: "1.3L Petrol" },
    features: ["Air Conditioning", "Cruise Control", "Apple CarPlay/Android Auto"],
    price: "49",
    image: "/mg3.jpg"
  },
  {
    name: "Hyundai Accent (Auto)",
    type: "Sedan",
    specs: { doors: 4, passengers: 5, luggage: "2 Large, 2 Small", engine: "1.6L GDI" },
    features: ["Air Conditioning", "Cruise Control", "Premium Steering"],
    price: "59",
    image: "/car-hero.svg"
  },
  {
    name: "Hyundai i30 (Auto)",
    type: "Hatchback",
    specs: { doors: 5, passengers: 5, luggage: "2 Large, 1 Small", engine: "1.8L GDI" },
    features: ["Rear View Camera", "Air Conditioning", "Cruise Control"],
    price: "65",
    image: "/car-hero.svg"
  },
  {
    name: "MG ZS (Auto)",
    type: "Compact SUV",
    specs: { doors: 4, passengers: 5, luggage: "2 Large, 2 Small", engine: "1.8L GDI" },
    features: ["Rear View Camera", "High Seating", "Cruise Control"],
    price: "75",
    image: "/mgzs.jpg"
  },
  {
    name: "Nissan X-Trail",
    type: "SUV (5/7 Seater)",
    specs: { doors: 4, passengers: "5-7", luggage: "3 Large, 2 Small", engine: "2.4L GDI" },
    features: ["Rear View Camera", "Premium Steering", "All-Mode 4x4-i"],
    price: "89"
  },
  {
    name: "Kia Carnival (Auto)",
    type: "People Mover",
    specs: { doors: 5, passengers: 8, luggage: "3 Large, 3 Small", engine: "3.3L V6" },
    features: ["Rear Park Assist", "Dual Zone AC", "8-Seater Luxury"],
    price: "110"
  }
];
